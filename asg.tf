...
resource "aws_launch_configuration" "three_tier_web_lconfig" {
  name_prefix = "three_tier_web_lconfig"
  image_id = "ami-03f4878755434977f"
  instance_type = "t2.micro"
  key_name = "three_tier_web_asg_kp"
...
resource "aws_autoscaling_group" "three_tier_web_asg" {
  name = "three_tier_web_asg"
  min_size = 2
  max_size = 3 
  desired_capacity = 2
  launch_configuration = aws_launch_configuration.three_tier_web_lconfig.id
  vpc_zone_identifier = [aws_subnet.three_tier_pub_sub_1.id, aws_subnet.three_tier_pub_sub_2.id]
}
